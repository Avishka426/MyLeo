...
interface EventPost {
  ...
  multipleDistrict?: { name: string; code: string; logo?: string };
}
...
export default function MultipleFeedScreen() {
  const { user } = useAuth();
  ...
  const isExco = user?.role === ROLES.MULTIPLE_EXCO;
      ...
      <Card>
        <View style={{ flexDirection: 'row', alignItems: 'flex-start', marginBottom: 8 }}>
          <View style={{ width: 28, height: 28, borderRadius: 14, overflow: 'hidden', backgroundColor: colors.accent + '33', justifyContent: 'center', alignItems: 'center', marginRight: 8 }}>
            {item.multipleDistrict?.logo
              ? <Image source={{ uri: item.multipleDistrict.logo }} style={{ width: 28, height: 28 }} />
              : <Ionicons name="globe" size={14} color={colors.accent} />}
          </View>
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 12, color: colors.textMuted, fontWeight: '600' }}>
              {item.multipleDistrict?.name ?? 'Multiple District'}
            </Text>
            <Text style={{ fontSize: 11, color: colors.textMuted }}>
    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      {isExco && tab === 'events' && (
        <TouchableOpacity
          ...
          onPress={() => router.push('/(multiple)/create-event' as any)}
          activeOpacity={0.85}
        >
