...
export default function ChatScreen() {
  ...
  const buildHistory = (msgs: Message[], excludeLastUser: boolean) => {
    const history = msgs
      .filter(m => !m.streaming && m.id !== 'welcome')
      .map(m => ({ role: m.role, content: m.content }));
    if (excludeLastUser && history.length > 0 && history[history.length - 1].role === 'user') {
      history.pop();
    }
    return history;
  };
  ...
  const send = () => {
    ...
    const history = buildHistory([...messages, userMsg], true);
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (json.type === 'token') {
            ...
            const citations: Citation[] = (json.citations ?? []).filter(
              (c: Citation) => c.relevance_score > 0.05
            );
            setMessages(prev =>
              prev.map(m =>
                m.id === asstId ? { ...m, content: finalContent, streaming: false, citations } : m
              )
            );
            setStreaming(false);
          }
    ...
    xhr.onloadend = () => {
      setStreaming(false);
      setMessages(prev =>
        prev.map(m => (m.id === asstId && m.streaming ? { ...m, streaming: false } : m))
      );
      setTimeout(() => flatRef.current?.scrollToEnd({ animated: true }), 100);
    };
            ...
            <View style={{ marginTop: 8 }}>
              ...
              {showCitations && item.citations.map((c, i) => (
                <View key={i} style={[styles.citation, { backgroundColor: colors.background, borderColor: colors.border }]}>
                  ...
                  <Text style={[styles.citationMeta, { color: colors.textMuted }]}>
                    Page {c.page_number}
                  </Text>
        ...
        <View style={[styles.inputRow, {
          ...
          <Pressable
            ...
            style={[styles.sendBtn, {
              backgroundColor: input.trim() && !streaming ? colors.primary : colors.border,
              borderRadius: radius.full,
            }]}
          >
